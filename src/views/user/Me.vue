...
const provinceSelect = ref<string>(userStore.user?.information[0].position_province ?? "未设置")
const citySelect = ref(userStore.user?.information[0].position_city ?? "未设置")
...
watch(
    ...
    (newUser) => {
      if (newUser?.information?.[0]) {
        provinceSelect.value = newUser.information[0].position_province || "未设置";
        citySelect.value = newUser.information[0].position_city || "未设置";
      }
    },
...
<template>
  <div class="w-full flex flex-col items-center">
    <div class="text-4xl font-bold text-gray-800 border-b-1 border-b-gray-200 w-full flex justify-center">
      <div class="max-w-5xl w-full p-12 pt-24">账户设置</div>
    </div>
    <div class="p-12 w-full max-w-5xl h-full flex flex-col gap-12 items-center justify-center">
      <div class="w-full h-full border-1 border-gray-200 rounded-lg overflow-hidden">
        ...
        <div class="flex justify-between items-center w-full px-6 py-4 border-t-1 border-t-gray-200">
      ...
      <div class="w-full h-full border-1 border-gray-200 rounded-lg overflow-hidden">
        ...
        <div class="w-full px-6 py-4 border-t-1 border-t-gray-200 text-sm text-gray-500">
          头像可选但强烈建议上传一个。
        </div>
      </div>
      <div class="w-full h-full border-1 border-gray-200 rounded-lg overflow-hidden">
